...
def find_path_triangulation(G, source, target_reduce, target_increase, get_fee, value, max_hops=None, max_fees=None):
    """
    target_reduce is the node we want to reduce our credit with (for value > 0)
    target_increase is the node which will result with an increased debt (for value > 0)
    source will owe less to target_reduce and target_increase will owe more to source (for value > 0)
    ...
    value may be negative, in which case
    source will owe more to target_reduce and target_increase will owe less to source
    """
    def get_fee_wrapper(b, a, value):
        # used to get the data from the graph in the right order and query the fees
        if b < a:
            return get_fee(b, a, G[b][a], value)
        else:
            return get_fee(b, a, G[a][b], value)
    ...
    def output(final_fee, intermediary_path):
    ...
    if value <= 0:
        ...
        raise ValueError('The value is greater than the amount source owes target_reduce : %d' % value)
    ...
    if value >= 0:
        ...
        return output(final_fee, intermediary_path)

    else:
        value = -value

        first_fee = get_fee_wrapper(source, target_increase, value)

        if max_hops is not None:
            max_hops -= 2

        intermediary_fee, intermediary_path = find_path(G, target_increase, target_reduce, get_fee,
                                                        value+first_fee, max_hops, max_fees, source)

        last_fee = get_fee_wrapper(target_reduce, source, value + intermediary_fee + first_fee)
